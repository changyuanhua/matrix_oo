...
struct sse_prefetch_priv {
    int values[4][4];
};

#define PRIV(x) \
    ((struct sse_prefetch_priv *) ((x)->priv))
...
bool sse_prefetch_mul(Matrix *dst, const Matrix *l, const Matrix *r)
{
...
#define SSE_PFDIST  8
                _mm_prefetch(&(PRIV(r)->values[k + SSE_PFDIST + 0][y]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + SSE_PFDIST + 1][y]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + SSE_PFDIST + 2][y]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + SSE_PFDIST + 3][y]), _MM_HINT_T1);
...
void print_sse_prefetch_matrix(Matrix *thiz)
{
    printf("sse_prefetchmatrix:\n");
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++)
...
MatrixAlgo SSEPREFETCHMatrixProvider = {
    .assign = assign,
    .equal = equal,
    .mul = sse_prefetch_mul,
    .print_matrix = print_sse_prefetch_matrix,
};
